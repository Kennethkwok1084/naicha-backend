...
class TestWeChatAuthAPI:
    ...
    async def test_bind_phone_success(self, async_client: AsyncClient, test_user: User):
        """测试绑定手机号成功"""
        with patch("app.api.routes.wechat_auth.WeChatAuthService") as mock_service_class:
            # 模拟服务返回
            mock_service = mock_service_class.return_value
    ...
    async def test_bind_phone_with_guest_session(self, async_client: AsyncClient, test_user: User):
        """测试带游客会话ID绑定手机号"""
        with patch("app.api.routes.wechat_auth.WeChatAuthService") as mock_service_class:
            mock_service = mock_service_class.return_value
            mock_service.bind_phone_number = AsyncMock(
