...
async def create_pos_order(
    ...
    if admin.role not in _POS_ALLOWED_ROLES:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Insufficient role for POS order creation.",
        )
...
async def get_admin_dashboard(
    ...
    try:
        payload = await dashboard_service.get_dashboard(range_key=range, compare=compare)
    except ValueError as exc:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail=str(exc),
